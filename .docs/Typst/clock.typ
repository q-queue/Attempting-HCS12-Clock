...
=== Clock AM-PM mode initial state which is enabled by default
...
#align(center)[
  #image("diagrams/Self-Mutating-Function-Reference.png", height: 36%)
]
...
- Wrapped in a callback to be be passed to a software counter
...
- The title rending counter piggy-back on the time rendering so that on the $n_("th")$  the time is updated the title changes too.
